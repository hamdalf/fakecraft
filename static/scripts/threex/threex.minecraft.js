...
THREEx.MinecraftChar = function (skinUrl) {
	...
	var geometry = new THREE.BoxGeometry(sizes.helmetH, sizes.helmetH, sizes.helmetH);
...
THREEx.MinecraftChar.baseUrl = '/scripts/threex/';
...
THREEx.MinecraftChar.prototype.loadWellKnownSkinRandomly = function(onLoad) {
	if (THREEx.MinecraftChar.wellKnownTextureIdxMap.length === 0) {
		for (var k in THREEx.MinecraftChar.skinWellKnownUrls) {
			THREEx.MinecraftChar.wellKnownTextureIdxMap.push(k);
		}
	}

	var randIdx = Math.round(Math.random() * (THREEx.MinecraftChar.wellKnownTextureIdxMap.length  - 1));
	var name = THREEx.MinecraftChar.wellKnownTextureIdxMap[randIdx];
	console.assert(THREEx.MinecraftChar.skinWellKnownUrls[name]);
	var url = THREEx.MinecraftChar.baseUrl + THREEx.MinecraftChar.skinWellKnownUrls[name];
	return this.loadSkin(url, onLoad);
};
...
THREEx.MinecraftChar.wellKnownTextureIdxMap = [];